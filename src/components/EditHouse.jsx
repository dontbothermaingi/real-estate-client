    ...
    useEffect(() => {
        ...
        .then((data) => {
            ...
            setFormData({
                ...
                year_built: new Date(data.year_built).toISOString().split("T")[0], // Formats to "YYYY-MM-DD"
                photos: data.photos,
        ...
        if (formData.photos && formData.photos.length > 0){
            formData.photos.forEach(photoObject => {
                if(photoObject.photo instanceof File){
                    ...
                    console.error("Invalid photo file:", photoObject.photo);
                }
            })
        }
    ...
    {isMobile ? (
        ...
        <Box>
            ...
            <Box paddingLeft={'300px'} paddingRight={'300px'} paddingTop={'20px'} paddingBottom={'30px'}>
                <Typography fontFamily={"GT Ultrabold"} fontSize={"40px"} color="black" textAlign={'center'}>EDIT A HOUSE LISTING</Typography>
                <form onSubmit={handleSubmit} style={{display:'flex', flexDirection:'column', backgroundColor:'white', borderRadius:'15px', border:'2px dashed #ddd', padding:'30px'}}>
                    ...
                    <Button type="submit" variant="contained" sx={{marginTop:'30px', margin:'auto',fontSize:'20px',fontFamily:"GT Bold", backgroundColor:'orange', color:"white", ":hover":{backgroundColor:'white', color:'orange'} }}>UPDATE HOUSE</Button>
